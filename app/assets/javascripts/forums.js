(function($, viewport) {
    ...
    var highlightBoxes = function() {
        if (!viewport.is("xs")) {
            ...
            $(
                ".forum-banner, .post-list, .js-toolbar-link, .js-filter-button"
            ).show();
...
$(document).on("click", ".filter-toolbar .close-filter-toolbar", function() {
    ...
    $(".forum-sidebar")
        ...
        .hide();
    $(
        ".forum-banner, .post-list, .js-toolbar-link, .js-open-filter-button"
    ).show();
...
$(document).on("change", "form.filter-form", function(e, where) {
    ...
    if (
        where !== "from_search_box" &&
        $(".post-filter").hasClass("js-on-mobile")
    ) {
...
$(document).on(
    "change",
    '.program-filters .filter-item input[type="checkbox"]',
    function() {
        var children = $(this)
            .closest(".filter-item")
...
function initPostForm() {
    $.each(
        $(
            ".chips input, .sleek_form .form-group input.form-control, .sleek_form .form-group select.form-control, .sleek_form .form-group textarea.form-control"
        ),
        function(index, input) {
            $(input)
                .parents(".form-group")
                .toggleClass("hasValue", input.value.length > 0);
        }
    );
...
function uploadFile(editor, selector, options) {
    ...
    $selector.bind("fileuploadfail", function(e, data) {
        ...
        alert(
            "Oops! Server could not handle your request at the moment. Please try again later."
        );
    });
}
...
SCEditor.prototype.createToolbar = function(items) {
    ...
    left_bar.innerHTML =
        "<a href='javascript:void(0)' id='markdown-write' class='toggle-markdown-preview active'>Write</a>" +
